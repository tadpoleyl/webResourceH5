                ...
                return this.each(function () {
                    $(this).on("touchstart", function (e) {
                        ...
                        if (endTime - startTime <= config.resposetime && Math.abs(endY - startY) < 6 && Math.abs(endX - startX) < 6 && fn) {
                            for (var i = 0; i < _this.listener["tap"].length; i++) {
                                _this.listener["tap"][i] && _this.listener["tap"][i](e, this);
                            }
                        }
                    });
                });
...
!function ($) {
    $.fn.slider = function (option) {
        ...
        var setting = $.extend(_default,option);
        new Slider({
            slider: this,
            callback: function (t) {
                setting.fnMove(t);
            },
            isNeedProcess: setting.isNeedProcess,
            bindMoveEventObj: setting.bindMoveEventObj
    ...
    function Slider(option) {
        ...
        this.bindMoveEventObj = option.bindMoveEventObj || this.slider.parent();
        this.slipping(this);
    ...
    Slider.prototype.creatScrollBar = function (parentHeight) {//生成滚动条
        var maxHeight = this.slider.height();//获取可移动元素的最大高度。
        var parentHeight = this.slider.parent().height();//获取可视区的最大高度
        var style = "";
        style = "<style>.ps{position:absolute;right:0%;height:" + parentHeight + "px;width:2px;background:#f90;top:0;opacity:0;-webkit-transition:400ms opacity;}\
    ...
    Slider.prototype.slipping = function (_this) {
       ...
       var maxHeight = self.slider.offset().height;//获取可移动元素的最大高度。
        ...
        if (self.isNeedProcess) {//判断是否需要滚动条；
            this.creatScrollBar(parentHeight);//生成滚动条
        }
        ...
        self.slider.on("touchstart mousedown", function (e) {
            ...
            var startY = e.pageY;
            self.lastY = e.pageY;
            self.bindMoveEventObj.on("touchmove mousemove", function (e) {
                e.preventDefault && e.preventDefault();
                var e = e.changedTouches ? e.changedTouches[0] : e;
                var endY = e.pageY;
                ...
                self.iSpeedY = e.pageY - self.lastY;
                self.lastY = e.pageY;
                ...
                $(this).off("touchend touchmove mousemove mouseup");

                var e = e.changedTouches ? e.changedTouches[0] : e;
                var endY = e.pageY;
                var t = endY - startY + currentY + self.iSpeedY * 14;
                if (t >= 0) {
                    t = 0;
                }
                else if (t <= parentHeight - maxHeight) {
                ...
                self.slider.css("-webkit-transition", "-webkit-transform " + Math.abs(self.iSpeedY) * 30 + "ms ease");
                self.slider.css("-webkit-transform", "translate3d(0," + t + "px,0)");
