define("flyPlugin", ["jquery"], function ($) {
    $.fn.extend({
        dragAndResize: function (operator, option, $compile, $scope, $location) {
            ...
            return this.each(function (i) {
                ...
                rightPoint1.css({ position: "absolute", left: -4, top: 0, width: pointSize , height: pointSize, borderRadius: "50%", zIndex: 1, background: pointColor, cursor: "url(static/images/mouserotate.ico),default", border: borderStyle });
                rightLine.append(rightPoint1);

                rightPoint2.css({ position: "absolute", left: -4, marginTop: -pointSize / 2, top: "50%", width: pointSize, height: pointSize, borderRadius: "50%", zIndex: 1, background: pointColor, cursor: "e-resize", border: borderStyle });
                rightLine.append(rightPoint2);

                rightPoint3.css({ position: "absolute", left: -4, bottom: 0, width: pointSize , height: pointSize, borderRadius: "50%", zIndex: 1, background: pointColor, cursor: "se-resize", border: borderStyle });
                ...
                s.on(START_EVENT, function (e) { ///改变位置
                    ...
                    oBody.on(MOVE_EVENT, function (event) {
                        ...
                        event = event.originalEvent.changedTouches[0] || event;
                ...
                rightPoint3.on(START_EVENT, function (e) { //同时改变宽高。

                    return; //禁用改变宽高
                    setting.sizeFn.start(e);
                    e.stopPropagation();
                    e.preventDefault();
                    var _this = this;
                    e = e.originalEvent.changedTouches[0] || e;
                    ...
                    var startWidth = s.width();
                    var startHeight = s.height();
                    var index = 0
                    oBody.on(MOVE_EVENT, function (event) {
                        event.stopPropagation();
                        event.preventDefault();
                        ...
                        var x = endX - startX + startWidth,
                            y = endY - startY + startHeight;
                        var startLeft = parseFloat(s.css("left")),
                            startTop = parseFloat(s.css("top"));
                        ...
                        if (setting.sizeFn.move(e)) { //是等比缩放的
                            s.width((x) / 32 + "rem");
                            s.height((x) / 32 * scale + "rem");
                            if (isTarget) {
                                target.width(x / 32 + "rem");
                                target.height(x / 32 * scale + "rem");
                            }
                        }
                        else {
                            isTarget && target.height(y / 32 + "rem").width(x / 32 + "rem");;
                            s.height(y / 32 + "rem").width((x) / 32 + "rem");

                        }
                        s.find("p").css({ width: x / 32 + "rem" });
                        return false;

                    }).on(END_EVENT, function (e) {
                        oBody.off(MOVE_EVENT + " " + END_EVENT);
                        index > 0 && setting.pubEnd(s);
                        setting.sizeFn.end(e);
        ...
        longTap: function (fn, opt) {
            ...
            if (device) {
                ...
                this[0].listener = this[0].listener || {};
                this[0].listener["longtap"] = this[0].listener["longtap"] || [];
                this[0].listener["longtaptype"] = this[0].listener["longtaptype"] || [];
                this[0].listener["longtap"].push(fn);
                var _this = this[0];
                var timer = null;
                this.on("touchstart", function (e) {
                    if (timer) {
                    ...
                    timer === null && (timer = setTimeout(function () {
                        for (var i = 0; i < _this.listener["longtap"].length; i++) {
                            _this.listener["longtap"][i] && _this.listener["longtap"][i](e, _this);
                        }
                    }, config.resposetime));
 ...
 */
    ...
    function createDefaultScrollbar(direction, interactive, type) {
        ...
        if (direction == 'h') {
            ...
            if (type === true) {
                scrollbar.style.cssText += ';width:7px;bottom:2px;top:2px;right:-4px';
